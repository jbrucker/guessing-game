...
This is a simple guessing game with a form-based graphical user-interface, using JavaFX for the U.I.
...
Applications with a graphical user interface (GUI) divide the code into at least 3 components,
...
* **Model** or Application, contains the logic (and code) for the application.  It usually has no knowledge of the View and Controller, but responds to requests (method calls) from them.  The Model typically consists of several classes and may itself be divided into components or layers.

And one other class:

* **Application** or **Main** class that starts the application.  It creates initial objects for model, view, and controller, and connects them as needed.
...
The attributes annotated with `@FXML` will be automatically set by JavaFX with a reference to a component in the UI with the same name (the "fx:id").  Its up to you to assign ids to components in the UI and make sure they match names in the controller class.
...
What to do with those events?  Your application has to attach **event handlers** (methods) to components in the UI.
Using SceneBuilder, you can specify what *events* should be handled by what *methods*.  

In the example code, the controller has methods `button1Press` and `button2Press` (you can use any names).  If you open the GameUI form in SceneBuilder, select button1, and open the "Code" toolbox, you'll see that the "On Action" event handler for button1 is the `button1Press` method.  The result is:
...
Event handler methods should be `public void` and may either have no parameter, or a parameter for the event type.  For Action Events, the parameter type is `ActionEvent`.  In GameController, we have:
```java
...
public void button1Press(ActionEvent event) {
    ...
    if (ok) {
        ...
        // get a hint from model and show it to user
...
The controller class (GameController) has a method named `initialize()` annotated with @FXML.  JavaFX calls this method after it creates the UI components. To make the UI more flexible, I set the text shown in the UI buttons and labels using the initialize method (instead of hardcoding strings into the fxml form).

The `initialize()` method also gets an initial message (hint) from the game and displays it in a label field.

## GameModel - the Model class

The Model class contains the logic for playing the game.
...
Your application logic **should not be in the controller**.
...
* [SceneBuilder 8.x from Gluon](http://gluonhq.com/products/scene-builder/) compiled from the most recent source code. There are 2 distributions:
  ...
  * [SceneBuilder Native App](http://gluonhq.com/products/scene-builder/) for Windows, Mac OSX, Linux, includes SceneBuilder and JRE.  Kind of a waste, since you already have a JRE.
...
You need to tell the IDE where the SceneBuilder JAR or executable is.
